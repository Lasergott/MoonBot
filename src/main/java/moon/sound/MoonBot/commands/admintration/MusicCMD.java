...
public class MusicCMD implements Command {
    ...
    private AudioEventListener audioEventListener = new AudioEventAdapter() {
        ...
        public void onTrackStart(AudioPlayer player, AudioTrack track) {
            guild.getTextChannelsByName(MoonBot.getInstance().getData().getMessage("generalChannel"), true).get(0).getManager()
                    .setTopic(track.getInfo()
                            .title)
                    .queue();
            try {
                ...
                guild.getTextChannelsByName(MoonBot.getInstance().getData().getMessage("generalChannel"), true).get(0).sendMessage(
                        eb.build()
                ).queue();
            ...
            new Timer().schedule(new TimerTask() {
                ...
                public void run() {
                    if (player.getPlayingTrack() == null) {
                        if (guild.getTextChannelsByName(MoonBot.getInstance().getData().getMessage("generalChannel"), true).size() > 0) {
                            guild.getTextChannelsByName(MoonBot.getInstance().getData().getMessage("generalChannel"), true).get(0).getManager().setTopic(
                                    MoonBot.getInstance().getData().getMessage("commandPrefix") + "music help"
                            ).queue();
                        }
                    }
                }
    ...
    public void action(String[] args, MessageReceivedEvent event) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "loop":
                try {
                    getTrackManager(guild).getQueuedTracks().stream().skip(0).forEach(t -> endlessList.add(t.getTrack()));
                    endlessMode = true;
